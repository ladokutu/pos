...
const Cart = (props) => {
  ...
  const [value, setValue] = useState('P-001');
  ...
  const [selectedValue, setSelectedValue] = React.useState('P-001');
  ...
  //const { DataCarts } = props;
	...
	const SimpanDataPaymentPos = async (event) => {
		...
		try { 
			...
			console.log(data_cart)
            var api='https://node.ladokutu.info/index.php/Posc/add_data_transaction_pos';  
			const response = await axios({
                  method: 'post',
                  url: api,
                  data: data_cart
			...
			if ((response.data.status === 200)&&(value === 'P-001')){
				setOpenPaymentPos(false)
				props.RemDataCart()
			}else if ((response.data.status === 200)&&(value === 'P-002')){
				setOpenPaymentPos(false)
